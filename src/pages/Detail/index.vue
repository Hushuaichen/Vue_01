      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- 输入框的失去焦点事件 blur 添加他 失去焦点就会触发这个事件 但是不好 因为他不会判断这次输入的数据和之前的数据是不是一样
                输入框的change事件包含了失去焦点事件 当用户失去焦点的时候 也会选择触发change事件 触发这个事件的前提是当前的这个数据必须和之前的数据bu一样的时候才会触发
                 -->
              ...
              <div class="add">
                <!-- //之前的点击就直接跳转 因为之前的不需要发请求 
                加入购物车不一样 当加入购物车的时候需要先发请求给后台 后台需要存储信息到数据库中  请求成功之后 后台会给我们返回信息
                根据这个信息再去跳转 否则有可能后台添加失败 而我们已经调到下个页面 就出问题了
                 -->
  ...
  export default {
    ...
    methods:{
      ...
      changeChecked(spuSaleAttrValueList,spuSaleAttrValue){
              ...
              //spuSaleAttrValue点击的那个属性值
              //排他 让所有的成员都变成同一种状态 
              spuSaleAttrValueList.forEach(item=>item.isChecked='0')
              //排他第二步 让当前点击的这一成员变成另一状态
                  spuSaleAttrValue.isChecked='1'
      },
      async addShopCart(){
        //第一步先发请求添加购物车
      //  const result=await this.$store.dispatch('addAddOrUpdateShopCart',{skuId:this.skuId,skuNum:this.skuNum}) 
        //第二步 根据请求添加购物车返回信息决定是否跳转到添加购物车页面
        // if(result==='ok'){
        //   //代表请求成功
        ...
        // }
        try {
          //只能通过dispatch的返回值 来判断请求是成功还是失败  dispatch调用的这个函数的返回值是promise
        await this.$store.dispatch('addAddOrUpdateShopCart',{skuId:this.skuId,skuNum:this.skuNum}) 


        //往添加购物车成功页面跳转的时候 需要带两个东西 skuNum和商品详情信息
        //skuNum是一个简单数据 可以通过路由传参query参数带过去
        //商品详情信息是一个复杂数据  用存储方案 sessionStorage
        alert('添加购物车成功')
        //localStorage(永久存储)  sessionStroage（关闭浏览器就不在了）
    ...
    computed:{
      ...
      imgList(){
        //如果skuInfo.skuImageList是undefined 我们应该传一个[]
