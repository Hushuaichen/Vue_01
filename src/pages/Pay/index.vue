  ...
  export default {
    ...
    methods:{
      ...
      async pay(){
        ...
        //轮询  隔两秒发一个请求 为了让后台返回订单的支付状态
        //判断用户有没有支付
          if (!this.timer) {  //if确保这个订单支付轮询只开启一个定时器
            this.timer=setInterval(async ()=>{
              const result=await this.$API.reqPayStatus(this.orderNum);
              if(result.code===200){
                //代表支付成功
                //关闭定时器  跳转  把成功的标志存储 用于用户点击按钮的时候判断
                this.payStatus=200
                this.$message.success('支付成功')

                clearInterval(this.timer)
                this.timer=null   //把之前设置定时器的编号id也清  这两个合起来才叫清除定时器
          ...
          //生成失败
          this.$message.error('生成图片链接失败'+error.message)
        }
